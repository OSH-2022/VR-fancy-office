...
namespace Dummiesman
{
    ...
    public class OBJLoader
    {
        ...
        public GameObject Load(Stream input, Color color=default,Vector3 position=default)
        {
            if (color == default) color = new Color(1, 1, 1, 0.5f);
            ...
            foreach (var builder in builderDict)
            {
                ...
                var builtObj = builder.Value.Build(color);
                builtObj.transform.SetParent(obj.transform, false);
                builtObj.transform.position = position;
            }
            return obj;
        }
        ...
        /// <returns>Returns a GameObject represeting the OBJ file, with each imported object as a child.</returns>
        public GameObject Load(Stream input, Stream mtlInput, Color color)
        {
            var mtlLoader = new MTLLoader();
            Materials = mtlLoader.Load(mtlInput);

            return Load(input,color);
        }
        ...
        /// <returns>Returns a GameObject represeting the OBJ file, with each imported object as a child.</returns>
        public GameObject Load(string path, string mtlPath, Color color,Vector3 position)
        {
        ...
        /// <param name="path">Input OBJ path</param>
        /// <returns>Returns a GameObject represeting the OBJ file, with each imported object as a child.</returns>
        public GameObject Load(String path, Color color=default,Vector3 position=default)
        {
            if (color == default) color = new Color(1, 1, 1, 0.5f);
            return Load(path, null, color,position);
        }
    }
}