...
在具体选择某一远程控制软件之前，第一步需要了解的是软件背后的技术，即软件使用的协议。只有选择了适合我们的高效控制协议才能选择更合适的开源框架进行开发。目前，主流可用的远程桌面控制协议如下：

#### X Protocol

X 协议是流行于类 Unix 操作系统上的可视化协议，最新版本是 X11。X Window System 是以其为基础的一套可视化系统。

X 协议看起来与其他软件非常不同：与用户直接交互的进程被称作服务器，而后台或远程提供计算的各进程被称作客户端。服务器在收集到用户的输入之后，会通过协议将数据传输给客户端，而客户端会将需要显示的数据使用协议告诉服务器，服务器依据这些数据向用户展示如这张图所示：

![](img/X_client_server_example.svg)

虽然该协议看似大大减小了网络负担，只需要传输远程计算得出的数据而其他协议中整个显示画面的视频流，但是 X 协议并不支持进程间通信，远程传输的数据依然是该进程生成画面的视频流。我们项目的特性决定了我们不会在本地计算太多内容，绝大多数数据来自远程，而使用 X 协议传输这些数据必然需要将几乎整个屏幕都在远程计算后用视频流回传。虽然 X 协议对视频流进行了很多优化，但是经过实验，其在处理复杂图形场景时性能相比较 VNC 并无明显提升。所以我们不计划采用 X 协议相关软件进行远程桌面控制。

#### NX Protocol

NX Technology 软件使用了自己的独有协议，从 X 协议进化而来。尽管如此，其核心技术已经开源，其客户端与服务器也均有一些开源实现。其从 X Window System 进化而来，针对远程视频传输进行优化，效率非常高。但是其致命的缺点是开源社区不太活跃，且其服务端与客户端的所有开源实现均不包含 Android。为 Android 重新写一份 NX 客户端并不是一件轻松的事情，且后文中的其他协议性能与 NX 差距并不会太大。所以我们不计划采用 NX 协议进行远程桌面控制。

#### Remote Desktop Protocol

Remote Desktop Protocol 是一款微软开发的 Windows 限定远程桌面连接协议，基于视频流。其针对 Windows 进行了大量优化，兼容性非常好，现代 Windows 版本中均内置 RDP 的服务端与客户端。虽然其没有对客户端的操作系统作限定，且 Android 操作系统中有较多的 RDP 开源客户端，但是考虑到 RDP 限制服务器的操作系统只能是 Windows，我们只有在其他协议均不适用时才会考虑 RDP。

#### Simple Protocol for Independent Computing Environment

SPICE 协议是一款内容简单的远程控制协议。其优点是实现非常简单，而缺点是效率非常低。正因如此，如今它已经几乎不用于在不同物理机之间的远程控制，而主要适用于虚拟机内外、模拟器内外的交互，例如 QEMU-KVM。所以我们不计划使用 SPICE 协议进行远程桌面控制。

#### Remote FrameBuffer Protocol

RFB 协议是一款开源、简单的远程控制协议，工作在帧缓冲器的层面上。正因如此，在大多数操作系统 - 包括安卓上，均有大量其服务端的开源实现。著名的 VNC 正是以其为基础协议工作。RFB 协议最大的缺点是其传输的数据是经过压缩后每个像素点，效率不如基于底层图形输出的 X 协议与基于桌面窗口的 RDP 协议。尽管如此，在我们的操作系统中，考虑到需要即时演算的动画量较大，X 协议与 RDP 协议在很大程度上会几乎退化成只传输像素点。实验表明，RFB 协议完全可以被我们很方便的采用。我们计划采用 RFB 协议作为远程控制协议。
...
这是一款开箱即用的远程桌面软件，基于 RFB 协议，覆盖了几乎所有常用的平台如linux，windows，mac，安卓。有一个用户友好的UI界面，支持文件传输。全部使用rust语言，并且代码目录下有介绍文件结构和实现思路，代码功能，有较好的可移植性。其优势大致如下：

* 高安全标准：银行标准和最新的 TLS 1.3 端到端加密技术可保护您的计算机免受未经授权的访问。
* 便捷的数据传输和 TCP 隧道 ：这使人能够在计算机之间传输文件，轻松设置 TCP 隧道，只需单击即可通过 TCP 隧道使用 RDP 连接。
* 原始应用程序：RustDesk 是使用 Rust 语言编写的开源软件，可以通过二进制文件、源代码或作为 docker 映像安装。
* 轻量级：作为开源产品，该软件提供了任何用户都可以安装的自由（例如，在 Linux 中，您无需成为 root 用户即可安装）。
* 有图像质量的定义 ： 可以通过调整比特率和量化器来定义您自己的图像质量。

![](./img/7.png)

#### freerdp

另一款开源的远程桌面软件是freerdp，采用的语言较多，实现的功能应该和rustdesk是类似的，基于 RDP，有远程应用、剪贴板重定向、多媒体重定向、磁盘重定向、并口重定向、串口重定向、打印机重定向、智能卡重定向、声音重定向、网络级身份验证 (NLA)、桌面组合、远程桌面网关、多点触控输入支持以及USB 重定向等功能。FreeRDP 的命令行语法基于本机 Microsoft RD 连接客户端语法。 它还可以处理由 Microsoft RD 连接客户端创建的 .rdp 连接文件。
