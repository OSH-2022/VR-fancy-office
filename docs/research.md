...
这是一款开箱即用的远程桌面软件，覆盖了几乎所有常用的平台如linux，windows，mac，安卓。有一个用户友好的UI界面，支持文件传输。全部使用rust语言，并且代码目录下有介绍文件结构和实现思路，代码功能，有较好的可移植性。其优势大致如下：
...
另一款开源的远程桌面软件是freerdp，采用的语言较多，实现的功能应该和rustdesk是类似的，有远程应用、剪贴板重定向、多媒体重定向、磁盘重定向、并口重定向、串口重定向、打印机重定向、智能卡重定向、声音重定向、网络级身份验证 (NLA)、桌面组合、远程桌面网关、多点触控输入支持以及USB 重定向等功能。FreeRDP 的命令行语法基于本机 Microsoft RD 连接客户端语法。 它还可以处理由 Microsoft RD 连接客户端创建的 .rdp 连接文件。
